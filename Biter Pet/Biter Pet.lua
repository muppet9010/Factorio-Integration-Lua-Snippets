/sc local playerName = "muppet9010"; local biterName = "%%USERNAME%%"; local biterDetailsColor = {1.0,1.0,1.0,1.0}; local biterDetailsSize = 1.5; local biterDeathMessageDuration = 600; local biterType = "behemoth-biter"; local closenessRange = 5; local exploringMaxRange = 15; local combatMaxRange = 50;
...
local followPlayerFunc = function(data);
    ...
    if biterPlayerDistance > data._exploringMaxRange + 1 then;
        if data._biter.distraction_command ~= nil then;
            if biterPlayerDistance > data._combatMaxRange then;
                ...
                if not data._fighting then;
                    if data._biterStateRenderId then; rendering.set_text(data._biterStateRenderId, data._biterStatusMessages_Fighting[math.random(#data._biterStatusMessages_Fighting)]); end; if data._debug then; data._player.print("biter started fighting far away - TEST - " .. game.tick); end data._fighting = true;
                end;
            end;
        ...
        if data._biter.distraction_command ~= nil then;
            if not data._fighting then;
                if data._biterStateRenderId then; rendering.set_text(data._biterStateRenderId, data._biterStatusMessages_Fighting[math.random(#data._biterStatusMessages_Fighting)]); end; if data._debug then; data._player.print("biter started fighting near by - TEST - " .. game.tick); end data._fighting = true;
            end;
        else;
